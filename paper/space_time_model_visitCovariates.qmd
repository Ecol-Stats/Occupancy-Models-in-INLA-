...
formula1 = inla.mdata(cbind(y,n),int_detection, scale_tod, scale_day) ~ 
...
func1 = function(...)
{
  ...
  rand = rnorm(length(pred1$group_elev), 0, 1/sqrt(theta[6]))
  aa + rand
}
func3_2 = function(...)
{
...
```{r}
npar = 3
tt = rbind(model1$summary.hyperpar[1:3,c(1,3,5)],
      model3_2$summary.hyperpar[1:3,c(1,3,5)],
      model3_3$summary.hyperpar[1:3,c(1,3,5)])
rownames(tt) = paste(rep(paste(1:3, sep = ""),each = 3),rep(c("alpha0", "alpha1", "alpha2"),3))
...
```{r}
npar = 2
tt = rbind(model3_2$summary.hyperpar[6:7,c(1,3,5)],
  model3_3$summary.hyperpar[5:6,c(1,3,5)])
kable(tt, booktabs = TRUE, digits = 2) %>% pack_rows(
