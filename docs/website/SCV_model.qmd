...
This code reproduces the case study "Occupancy model with
spatially-varying coefficients for the Gray Catbird" in "Spatio-temporal
Occupancy Models with INLA". The dataset and part of the code for
preparing the data, is taken from the supplementary material of
[@doser2022]
...
Here are plots of the spatial temporal trends according to the three
models.
...
trend2 %>% ggplot() + geom_tile(aes(x,y,fill = value)) +
  facet_wrap(.~name) + coord_equal() +
