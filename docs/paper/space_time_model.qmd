...
buffer_25 <- st_buffer(df_sf, dist = .75) 
...
ggplot()+geom_sf(data=df_sf)+
  facet_wrap(~year)+
  geom_sf(data=df_sf[loocv_m1_auto$groups[[200]]$idx,],color="orange")+
  geom_sf(data = df_sf[200,],colour="purple") 
  

data.frame(auto= c(ULOOCV_auto,ULOOCV2_auto,ULOOCV3_auto),manual=c(ULOOCV_manual,ULOOCV2_manual,ULOOCV3_manual))
...
d1 = data.frame(time = time,
                ...
                m3 = apply(inla.link.logit(det3,inverse = T) , 1,mean),
