...
cbind(c(model$waic$waic,model2$waic$waic,model3$waic$waic,model4$waic$waic),
      c(model$dic$dic,model2$dic$dic,model3$dic$dic,model4$dic$dic),
      c(model$mlik[1,1],model2$mlik[1,1],model3$mlik[1,1],model4$mlik[1,1]))
...
svc2 = inla.posterior.sample.eval(function(x) -(A_pred %*% i2)[,1], 
                                 samples2)
svc3 = inla.posterior.sample.eval(function(x) -(bcr_idx[coords.0.sf$bcr_idx]), 
                                  samples3)
...
dd = data.frame(x = coords.0[,1],
                ...
                z1_3 = apply(svc3,1,mean),
                q1_3 = apply(svc3,1,quantile,0.025, na.rm =T),
                q2_3 = apply(svc3,1,quantile,0.975, na.rm =T),
...
dd = dd %>%
  ...
  mutate(check2 = case_when( q1_2>=0 & q2_2>=0 ~ 1,
                             q1_2<0 & q2_2<0 ~ -1,
                             q1_2<0 & q2_2>0 ~ 0)) %>%
  mutate(check3 = case_when( q1_3>=0 & q2_3>=0 ~ 1,
                             q1_3<0 & q2_3<0 ~ -1,
                             q1_3<0 & q2_3>0 ~ 0)) %>%
...
geom_sf(data = within.sf.list[[which(sp.names == curr.sp)]], alpha = 0, col = 'black') + 
  ...
  guides(colour = guide_colourbar(barheight = unit(10, "cm")))
p3 =  ggplot() +
  geom_stars(data = pred.stars, aes(x = x, y = y, fill = z1_3), interpolate = TRUE) +
  #geom_sf(data = east.us, alpha = 0, col = 'grey') +
  geom_sf(data = bcrs.ne.grouped, alpha = 0, color = 'grey') +
  geom_sf(data = within.sf.list[[which(sp.names == curr.sp)]], alpha = 0, col = 'black') + 
  scale_fill_scico(name= "Mean \ntemporal trend" , limits = lims) + 
...
p1 + p2 + p4 + p11 +p22 +p33 + plot_layout(ncol = 3,  guides = "collect") +
...
trend2 <-  dd |> 
  ...
  # )) |>
  terra::rast() |> 
  rename(`SVCM 1` = check1,
         `SVCM 2` = check2,
         `SVCM 3` = check4)

values(trend2$`SVCM 1`) <- as.factor(values(trend2$`SVCM 1`))
values(trend2$`SVCM 2`) <- as.factor(values(trend2$`SVCM 2`))
values(trend2$`SVCM 3`) <- as.factor(values(trend2$`SVCM 3`))
...
Predictions over the whole domain of the linear predictor for occurrence and probability of occurrence. These are for year 2019.
