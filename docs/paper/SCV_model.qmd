...
buffer_25 <- st_buffer(df_sf, dist = 500) 
...
ggplot()+geom_sf(data=df_sf)+
  facet_wrap(~year)+
  geom_sf(data=df_sf[I_i[[10000]],],color="orange")+
  geom_sf(data = df_sf[10000,],colour="purple") +
  geom_sf(data=buffer_25[10000,],color="red",alpha=0) 
  


loocv_m2 <- inla.group.cv(result = model2,groups = I_i)

ULOOCV = mean(log(loocv_m1$cv[1:n]))

filter_and_plot <- function(i, df_sf, buffer_25, time= 1) {
  df_sf_subset <- df_sf %>% 
    filter(between(year, left = df_sf$year[i]-time, right = df_sf$year[i]+time)) 
  
  Buffer_i <- df_sf_subset %>% st_intersects(buffer_25[i,], sparse = FALSE) %>% unlist()
  
  I_i <- df_sf_subset[Buffer_i,] %>% pull(id)
  
  return(I_i)
}

I_i <- lapply(1:nrow(df_sf), function(i) filter_and_plot(i, df_sf, buffer_25,time=2))
